#!/bin/bash
# Initialize a Vite + React + JavaScript + Tailwind + shadcn/ui project
# Usage: bash scripts/init-vite-js.sh <project-name>
...
echo "🚀 Creating Vite project (JavaScript): $PROJECT_NAME"

# Create Vite project with JavaScript template
...
# Create utils (JavaScript version)
mkdir -p src/lib
...
# Create Button component (JavaScript version)
...
# Update vite.config.js with path aliases (JavaScript version)
...
# Update components.json for shadcn CLI (JavaScript version)

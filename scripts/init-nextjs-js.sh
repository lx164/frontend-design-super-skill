#!/bin/bash
# Initialize a Next.js + JavaScript + Tailwind + shadcn/ui project
# Usage: bash scripts/init-nextjs-js.sh <project-name>
...
echo "🚀 Creating Next.js project (JavaScript): $PROJECT_NAME"

# Create Next.js project (JavaScript version, without --typescript flag)
...
# Create site config (JavaScript version)
cat > src/config/site.js << 'EOF'
...
# Create a basic page with animation example (JavaScript/JSX version)
